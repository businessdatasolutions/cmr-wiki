...
- [[cmr-framework]] — **integrative synthesis.** Capability–Market–Risk business-architecture model: an explore/exploit capability engine (+ Management governor) turning on a continuous-learning helix (the DevOps ∞ loop translated to a 7-phase Scout→Sense→Seize→Transfer→Deploy→Monitor→Transform cycle) produces the value drivers Profit / Risk / Growth → Value. Marries ambidexterity, dynamic capabilities, DevOps, Strategyzer's explore→Transfer→exploit lifecycle, and strategic-risk-as-value-driver. User-originated; confidence 0.72.
