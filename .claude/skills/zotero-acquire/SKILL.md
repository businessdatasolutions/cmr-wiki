...
- After the user has dragged the sources they want into the dedicated **`cmr-wiki`** Zotero collection
...
3. A dedicated collection (default name **`cmr-wiki`**) holding the items to acquire.
...
uv run --with-requirements requirements.txt python fetch_zotero.py --collection cmr-wiki --dry-run

# Acquire for real:
uv run --with-requirements requirements.txt python fetch_zotero.py --collection cmr-wiki
```
...
| --- | --- | --- |
| `-c`, `--collection` | `cmr-wiki` | Zotero collection name (case-insensitive) |
...
1. User drags sources into the `cmr-wiki` Zotero collection.
2. Run `--dry-run`, review routing, then run for real. This is **Acquire** — log it as
   `acquire | zotero-batch cmr-wiki (<n> items)` only if you stop here (per CLAUDE.md §Acquire).
...
notes: |
  Acquired from Zotero collection `cmr-wiki`. itemType ... routed to raw/<type>/ — confirm at Process.
---
...
- **`collection not found: 'cmr-wiki'`** — name typo, or the collection doesn't exist yet. The error
